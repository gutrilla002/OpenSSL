...
signing using the algorithm given by I<algo> and the key given through
L<EVP_PKEY_CTX_new(3)> or L<EVP_PKEY_CTX_new_from_pkey(3)>.  A context I<ctx>
without a pre-loaded key cannot be used with this function.
This function enables using algorithms that can process infinite input, such
as ED25519, RSA-SHA256 and similar.

EVP_PKEY_sign_update() adds I<inlen> bytes from I<in> to the data to be
...
EVP_PKEY_sign_final() signs the processed data and places the data in I<sig>,
and the number of signature bytes in I<*siglen>, if the number of bytes
doesn't surpass the size given by I<sigsize>.
I<sig> may be NULL, and in that case, only I<*siglen> is updated with the
number of signature bytes.

EVP_PKEY_sign() is a one-shot function that performs the same thing as single
EVP_PKEY_sign_update() call with I<in> and I<inlen> followed by
EVP_PKEY_sign_final() with I<sig> and I<siglen>, and where I<*siglen> serves
as I<sigsize>.  I<*siglen> must therefore be pre-initialized with the size of
the buffer I<sig> points at.

EVP_PKEY_sign_ex() does the same thing as EVP_PKEY_sign(), but instead of
expecting I<*siglen> to have the size of I<sig>, its size is passed through
I<sigsize>.

=head1 NOTES

Some signature implementations only accumulate the input data and does no
...
and signing the result.  The caller is expected to know how the chosen
algorithm is supposed to behave.

For example, an RSA implementation can be expected to only accumulate the
input data with no further processing, i.e. to expect a digest as input, while
ED25519 or RSA-SHA256 can be expected to process the input with a hash, i.e.
to produce the digest internally.
...
parameter "digest" (B<OSSL_SIGNATURE_PARAM_DIGEST>) can not be used to combine
a signature algorithm with a hash algorithm to process the input.  In other
...
EVP_PKEY_sign_final() resets the context I<ctx> in such a way that another
sequence of (multiple) EVP_PKEY_sign_update() and EVP_PKEY_sign_final() can be
performed using the parameters that was given by the initializing call.
Likewise, EVP_PKEY_sign() can be called more than once on the same context to
have several one-shot operations are performed using the same parameters.

=head1 RETURN VALUES

All functions return return 1 for success and 0 or a negative value for failure.
...
=head2 RSA-SHA256 with PKCS#1 padding

Sign a document using RSA-SHA256 with PKCS#1 padding:
