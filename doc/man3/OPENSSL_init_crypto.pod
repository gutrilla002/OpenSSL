...
Disables automatic execution of OPENSSL_cleanup() via library destructor.
Starting from version 3.4 OpenSSL no longer uses atexit(3) handler to call
OPENSSL_cleanup() automatically.  Instead of using atexit(3) the OpenSSL
relies on run time linker to call OPENSSL_cleanup() via library destructor. 
